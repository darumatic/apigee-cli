...
def _create_api_product(username, password, mfa_secret, org, profile, name, body, **kwargs):
    return Apiproducts(gen_auth(username, password, mfa_secret), org, name) \
...
def _list_api_products(
    ...
    return Apiproducts(gen_auth(username, password, mfa_secret), org, None).list_api_products(
        prefix=prefix,
        expand=expand,
        count=count,
        startkey=startkey,
...
@click.option("--count", type=click.INT, default=1000, show_default=True, help="Number of API products to return in the API call. The maximum limit is 1000. Use with the startkey to provide more targeted filtering.")
...
@apiproducts.command(help="Updates an existing API product. You must include all required values, whether or not you are updating them, as well as any optional values that you are updating.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="name", required=True)
...
def _push_apiproducts(username, password, mfa_secret, org, profile, file, **kwargs):
    return Apiproducts(gen_auth(username, password, mfa_secret), org, None).push_apiproducts(
        file
    )
