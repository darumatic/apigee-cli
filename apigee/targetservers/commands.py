...
def _create_a_targetserver(username, password, mfa_secret, org, profile, name, environment, body, **kwargs):
    return Targetservers(gen_auth(username, password, mfa_secret), org, name) \
        .create_a_targetserver(environment, body) \
        .text

@targetservers.command(help="Create a TargetServer in the specified environment. TargetServers are used to decouple TargetEndpoint HTTPTargetConnections from concrete URLs for backend services.")
@common_auth_options
...
def _list_targetservers_in_an_environment(
    ...
    return Targetservers(
        gen_auth(username, password, mfa_secret), org, None
...
def _push_targetserver(username, password, mfa_secret, org, profile, environment, file, **kwargs):
    return Targetservers(gen_auth(username, password, mfa_secret), org, None).push_targetserver(
        environment, file
    )
