...
class ParentParser:
    ...
    def _build_mfa_secret_argument(self):
        ...
        self._parent_parser.add_argument('--mfa-secret', action='store', help='apigee mfa secret', required=False, default=APIGEE_MFA_SECRET if mfa_secret is None else mfa_secret)

    def _build_org_argument(self):
        org = authorization.get_credential(self._profile, 'org')
        if org:
            self._parent_parser.add_argument('-o', '--org', action='store', help='apigee org', required=False, default=org)
        elif APIGEE_ORG:
            self._parent_parser.add_argument('-o', '--org', action='store', help='apigee org', required=False, default=APIGEE_ORG)
        else:
            self._parent_parser.add_argument('-o', '--org', action='store', help='apigee org', required=True)

    def _build_username_argument(self):
        username = authorization.get_credential(self._profile, 'username')
        if username:
            self._parent_parser.add_argument('-u', '--username', action='store', help='apigee username', required=False, default=username)
        elif APIGEE_USERNAME:
            self._parent_parser.add_argument('-u', '--username', action='store', help='apigee username', required=False, default=APIGEE_USERNAME)
        else:
            self._parent_parser.add_argument('-u', '--username', action='store', help='apigee username', required=True)

    def _build_password_argument(self):
        password = authorization.get_credential(self._profile, 'password')
        if password:
            self._parent_parser.add_argument('-p', '--password', action='store', help='apigee password', required=False, default=password)
        elif APIGEE_PASSWORD:
            self._parent_parser.add_argument('-p', '--password', action='store', help='apigee password', required=False, default=APIGEE_PASSWORD)
        else:
            self._parent_parser.add_argument('-p', '--password', action='store', help='apigee password', required=True)
