...
def _create_developer(
    username, password, mfa_secret, org, profile, name, first_name, last_name, user_name, attributes='{"attributes" : [ ]}', **kwargs
):
    return Developers(gen_auth(username, password, mfa_secret), org, name) \
        .create_developer(
            first_name,
            last_name,
            user_name,
...
@click.option("-n", "--name", help="The developer's email. This value is used to uniquely identify the developer in Apigee Edge.", required=True)
...
@click.option("--attributes", default='{"attributes" : [ ]}', required=True, help="request body e.g.: '{\"attributes\" : [ ]}'")
...
@developers.command(help="Deletes a developer from an organization. All apps and API keys associated with the developer are also removed from the organization. All times in the response are UNIX times. To avoid permanently deleting developers and their artifacts, consider deactivating developers instead.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="The developer's email. This value is used to uniquely identify the developer in Apigee Edge.", required=True)
...
@developers.command(help="Returns the profile for a developer by email address or ID. All time values are UNIX time values. The profile includes the developer's email address, ID, name, and other information.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="The developer's email. This value is used to uniquely identify the developer in Apigee Edge.", required=True)
...
def _get_developer_by_app(username, password, mfa_secret, org, profile, app_name, **kwargs):
    return Developers(gen_auth(username, password, mfa_secret), org, None) \
        .get_developer_by_app(app_name) \
        .text

@developers.command(help="Gets the developer profile by app name. The profile retrieved is for the developer associated with the app in the organization. All time values are UNIX time values.")
@common_auth_options
@common_verbose_options
@common_silent_options
...
def _list_developers(
    username, password, mfa_secret, org, profile, prefix=None, expand=False, count=1000, startkey="", **kwargs
):
    return Developers(gen_auth(username, password, mfa_secret), org, None).list_developers(
        prefix=prefix,
        expand=expand,
        count=count,
        startkey=startkey,
...
@developers.command(help="Lists all developers in an organization by email address. This call does not list any company developers who are a part of the designated organization.")
@common_auth_options
@common_verbose_options
@common_silent_options
...
@click.option("--count", type=click.INT, default=1000, show_default=True, help="Limits the list to the number you specify. Use with the startKey parameter to provide more targeted filtering. The limit is 1000.")
@click.option("--startkey", default="", show_default=True, help="To filter the keys that are returned, enter the email of a developer that the list will start with.")
...
@developers.command(help="Sets a developer's status to active or inactive for a specific organization. Run this API for each organization where you want to change the developer's status.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="The developer's email. This value is used to uniquely identify the developer in Apigee Edge.", required=True)
@click.option('--action',
              type=click.Choice(['active', 'inactive'], case_sensitive=False), required=True, show_default=True)
...
@developers.command(help="Updates or creates developer attributes. This API replaces the current list of attributes with the attributes specified in the request body. This lets you update existing attributes, add new attributes, or delete existing attributes by omitting them from the request body.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="The developer's email. This value is used to uniquely identify the developer in Apigee Edge.", required=True)
