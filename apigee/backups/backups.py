...
class Backups:
    ...
    def __init__(self, auth, org_name, target_directory, prefix=None, fs_write=False, apis=None, environments=['test', 'prod']):
        self.auth = auth
        self.org_name = org_name
        self.target_directory = resolve_target_directory(target_directory)
        self.prefix = prefix
        self.fs_write = fs_write
        self.apis = apis or APIS
        self.environments = environments
        self.snapshot_data = Struct(apis={}, keyvaluemaps={}, targetservers={}, caches={}, developers=[], apps={}, apiproducts=[], userroles=[])
        ...
        for developer in self.snapshot_data.developers:
            try:
                write_file(
                    ...
                    str(Path(self.target_directory) / self.org_name / 'developers' / (developer + '.json')),
                    fs_write=self.fs_write,
                )
            except HTTPError as e:
