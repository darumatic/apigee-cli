...
class Backups:
    ...
    def __init__(self, auth, org_name, target_directory, prefix=None, fs_write=False, apis=APIS, environments=['test', 'prod']):
        self.auth = auth
    ...
    def download_apis_snapshot(self):
        for api in Apis(self.auth, self.org_name, None).list_api_proxies(prefix=self.prefix, format='dict'):
            ...
            write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'], indent=data['indent'])
    ...
    def download_apis(self):
        for api, metadata in self.snapshot_data.apis.items():
            for revision in metadata['revision']:
                output_file = self._gen_download_path(subpaths=['apis', api, revision, f'{api}.zip'])
                target_directory = os.path.dirname(output_file)
                try:
                    Apis(self.auth, self.org_name, None).export_api_proxy(api, revision, fs_write=True, output_file=output_file)
    ...
    def download_keyvaluemaps_snapshot(self):
        for environment in self.environments:
            try:
                self.snapshot_data.keyvaluemaps[environment] = Keyvaluemaps(self.auth, self.org_name, None).list_keyvaluemaps_in_an_environment(
            ...
            data = {
                ...
                'target_path': self._gen_snapshot_path(subpaths=['keyvaluemaps', environment, 'keyvaluemaps.json']),
                'fs_write': self.fs_write,
                'indent': 2,
            }
            write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'], indent=data['indent'])
    ...
    def download_keyvaluemaps(self):
        for environment in self.environments:
            for kvm in self.snapshot_data.keyvaluemaps[environment]:
                try:
                    data = {
                        'snapshot': Keyvaluemaps(self.auth, self.org_name, kvm).get_keyvaluemap_in_an_environment(environment).text,
                        'target_path': self._gen_download_path(subpaths=['keyvaluemaps', environment, f'{kvm}.json']),
                        'fs_write': self.fs_write,
                    }
    ...
    def download_targetservers_snapshot(self):
        for environment in self.environments:
            try:
                self.snapshot_data.targetservers[environment] = Targetservers(self.auth, self.org_name, None).list_targetservers_in_an_environment(
                    environment, prefix=self.prefix, format='dict'
                )
            except HTTPError:
            ...
            data = {
                ...
                'target_path': self._gen_snapshot_path(subpaths=['targetservers', environment, 'targetservers.json']),
                'fs_write': self.fs_write,
                'indent': 2,
            }
            write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'], indent=data['indent'])
    ...
    def download_targetservers(self):
        for environment in self.environments:
            for targetserver in self.snapshot_data.targetservers[environment]:
                try:
                    data = {
                        'snapshot': Targetservers(self.auth, self.org_name, targetserver).get_targetserver(environment).text,
                        'target_path': self._gen_download_path(subpaths=['targetservers', environment, f'{targetserver}.json']),
                        'fs_write': self.fs_write,
                    }
                    write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'])
                except HTTPError as e:
    ...
    def download_caches_snapshot(self):
        for environment in self.environments:
            try:
                self.snapshot_data.caches[environment] = Caches(self.auth, self.org_name, None).list_caches_in_an_environment(
                    environment, prefix=self.prefix, format='dict'
                )
            except HTTPError:
            ...
            data = {
                ...
                'target_path': self._gen_snapshot_path(subpaths=['caches', environment, 'caches.json']),
                'fs_write': self.fs_write,
                'indent': 2,
            }
            write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'], indent=data['indent'])
    ...
    def download_caches(self):
        for environment in self.environments:
            for cache in self.snapshot_data.caches[environment]:
                try:
                    data = {
                        'snapshot': Caches(self.auth, self.org_name, cache).get_information_about_a_cache(environment).text,
                        'target_path': self._gen_download_path(subpaths=['caches', environment, f'{cache}.json']),
                        'fs_write': self.fs_write,
                    }
                    write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'])
                except HTTPError as e:
    ...
    def download_developers(self):
        for developer in self.snapshot_data.developers:
            try:
                data = {
                    'snapshot': Developers(self.auth, self.org_name, developer).get_developer().text,
                    'target_path': self._gen_download_path(subpaths=['developers', f'{developer}.json']),
                    'fs_write': self.fs_write,
                }
                write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'])
            except HTTPError as e:
                console.echo(f'Ignoring {type(e).__name__} {e.response.status_code} error for Developer ({developer})')
    ...
    def download_apiproducts(self):
        for apiproduct in self.snapshot_data.apiproducts:
            try:
                data = {
                    'snapshot': Apiproducts(self.auth, self.org_name, apiproduct).get_api_product().text,
                    'target_path': self._gen_download_path(subpaths=['apiproducts', f'{apiproduct}.json']),
                    'fs_write': self.fs_write,
                }
                write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'])
            except HTTPError as e:
    ...
    def download_apps_snapshot(self, expand=False, count=1000, startkey=""):
        ...
        for app, details in self.snapshot_data.apps.items():
            data = {'snapshot': details, 'target_path': self._gen_snapshot_path(subpaths=['apps', f'{app}.json']), 'fs_write': self.fs_write, 'indent': 2}
            write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'], indent=data['indent'])
    ...
    def download_apps(self):
        for developer, apps in self.snapshot_data.apps.items():
            for app in apps:
                try:
                    data = {
                        'snapshot': Apps(self.auth, self.org_name, app).get_developer_app_details(developer).text,
                        'target_path': self._gen_download_path(subpaths=['apps', developer, f'{app}.json']),
                        'fs_write': self.fs_write,
                    }
                    write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'])
                except HTTPError as e:
    ...
    def _get_permissions(self, role_name):
        return json.dumps(
            Backups._sort_lists_in_permissions(Permissions(self.auth, self.org_name, role_name).get_permissions(formatted=True, format='json')), indent=2
        )
    ...
    def _download_users_for_a_role(self, role_name):
        try:
            data = {
                ...
                'target_path': self._gen_download_path(subpaths=['userroles', role_name, 'users.json']),
                'fs_write': self.fs_write,
                'indent': 2,
            }
            write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'], indent=data['indent'])
    ...
    def _download_resource_permissions(self, role_name):
        try:
            data = {
                ...
                'target_path': self._gen_download_path(subpaths=['userroles', role_name, 'resource_permissions.json']),
                'fs_write': self.fs_write,
            }
            write_file(data['snapshot'], data['target_path'], fs_write=data['fs_write'])
        except HTTPError as e:
            console.echo(f'Ignoring {type(e).__name__} {e.response.status_code} error for User Role ({role_name}) resource permissions')
