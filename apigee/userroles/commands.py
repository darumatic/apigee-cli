...
def _add_a_user_to_a_role(username, password, mfa_secret, org, profile, name, user_email, **kwargs):
    return Userroles(gen_auth(username, password, mfa_secret), org, name) \
...
def _create_a_user_role_in_an_organization(username, password, mfa_secret, org, profile, names, **kwargs):
    return Userroles(gen_auth(username, password, mfa_secret), org, names) \
...
@userroles.command(help="Removes a permission from a resource for the role specified. Permissions are case sensitive. Specify the permission as get, put, or delete.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="the role name", required=True)
...
def _get_resource_permissions_for_a_specific_role(username, password, mfa_secret, org, profile, name, resource_path="", **kwargs):
    return Userroles(gen_auth(username, password, mfa_secret), org, name) \
        .get_resource_permissions_for_a_specific_role(
...
def _verify_a_user_roles_permission_on_a_specific_RBAC_resource(
    username, password, mfa_secret, org, profile, name, permission, resource_path, **kwargs
):
    return Userroles(gen_auth(username, password, mfa_secret), org, name) \
