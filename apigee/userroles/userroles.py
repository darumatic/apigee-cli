...
DELETE_A_PERMISSION_FOR_A_RESOURCE_PATH = (
    '{api_url}/v1/organizations/{org}/userroles/{role_name}/permissions/{permission}?path={resource_path}'
)
DELETE_RESOURCE_FROM_PERMISSIONS_PATH = (
    '{api_url}/v1/organizations/{org}/userroles/{role_name}/permissions?path={resource_path}&delete=true'
)
...
class Userroles:
    ...
    def add_a_user_to_a_role(self, user_email):
        uri = ADD_A_USER_TO_A_ROLE_PATH.format(
            api_url=APIGEE_ADMIN_API_URL, org=self._org_name, role_name=self._role_name, user_email=user_email
        )
    ...
    def delete_a_permission_for_a_resource(self, permission, resource_path):
        ...
        hdrs = auth.set_header(
            self._auth, headers={'Accept': 'application/json', 'Content-Type': 'application/octet-stream'}
        )
