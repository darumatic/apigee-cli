...
def _get_api_proxy_deployment_details(username, password, mfa_secret, org, profile,
    name, format, showindex, tablefmt, revision_name_only, **kwargs
):
...
@deployments.command(help="Returns detail on all deployments of the API proxy for all environments. All deployments are listed in the test and prod environments, as well as other environments, if they exist.", aliases=["get-api-proxy-deployment-details"])
@common_auth_options
...
@click.option("--format", help="defines how to format output when using the -r flag",
              default="table", type=click.Choice(["json", "table"], case_sensitive=False))
@click.option('--showindex/--no-showindex', default=False)
@click.option('--tablefmt', help="defines how the table is formatted",
