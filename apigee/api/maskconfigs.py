...
class Maskconfigs(IMaskconfigs):
    ...
    def create_data_masks_for_an_api_proxy(self, request_body):
        uri = '{0}/v1/organizations/{1}/apis/{2}/maskconfigs'.format(APIGEE_ADMIN_API_URL, self._org_name, self._api_name)
    ...
    def delete_data_masks_for_an_api_proxy(self, maskconfig_name):
        uri = '{0}/v1/organizations/{1}/apis/{2}/maskconfigs/{3}'.format(APIGEE_ADMIN_API_URL, self._org_name, self._api_name, maskconfig_name)
    ...
    def list_data_masks_for_an_organization(self):
        uri = '{0}/v1/organizations/{1}/maskconfigs'.format(APIGEE_ADMIN_API_URL, self._org_name)
        hdrs = authorization.set_header({'Accept': 'application/json'}, self._auth)
        resp = requests.get(uri, headers=hdrs)
        resp.raise_for_status()
        # print(resp.status_code)
