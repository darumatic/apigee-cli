...
class Apps:
    ...
    def create_developer_app(self, developer, request_body):
        uri = CREATE_DEVELOPER_APP_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer)
    ...
    def delete_developer_app(self, developer):
        uri = DELETE_DEVELOPER_APP_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer, name=self._app_name)
    ...
    def create_empty_developer_app(self, developer, display_name="", callback_url=""):
        uri = CREATE_EMPTY_DEVELOPER_APP_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer)
        hdrs = auth.set_header(self._auth, headers={'Accept': 'application/json', 'Content-Type': 'application/json'})
        body = {
            'name': self._app_name,
    ...
    def get_developer_app_details(self, developer):
        uri = GET_DEVELOPER_APP_DETAILS_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer, name=self._app_name)
        hdrs = auth.set_header(self._auth, headers={'Accept': 'application/json'})
        resp = requests.get(uri, headers=hdrs)
        resp.raise_for_status()
        return resp

    def list_developer_apps(self, developer, prefix=None, expand=False, count=1000, startkey="", format='json'):
        uri = LIST_DEVELOPER_APPS_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer)
        if expand:
            uri += f'?expand={expand}'
        else:
    ...
    def list_apps_for_all_developers(self, list_of_developers, prefix=None, expand=False, count=1000, startkey="", format='dict', progress_bar=False):
        apps = {}
        for developer in list_of_developers:
            apps[developer] = self.list_developer_apps(developer, prefix=prefix, expand=expand, count=count, startkey=startkey, format=format)
        return apps

    def delete_key_for_a_developer_app(self, developer, consumer_key):
        uri = DELETE_KEY_FOR_A_DEVELOPER_APP_PATH.format(
            api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer, name=self._app_name, consumer_key=consumer_key
        )
    ...
    def create_a_consumer_key_and_secret(
        self, developer, consumer_key=None, consumer_secret=None, key_length=32, secret_length=32, key_suffix=None, key_delimiter='-', products=[]
    ):
        uri = CREATE_A_CONSUMER_KEY_AND_SECRET_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer, name=self._app_name)
        hdrs = auth.set_header(self._auth, headers={'Accept': 'application/json', 'Content-Type': 'application/json'})
