...
class Apps:
    ...
    def delete_developer_app(self, developer):
        uri = DELETE_DEVELOPER_APP_PATH.format(
            api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer, name=self._app_name
        )
    ...
    def list_apps_for_all_developers(
        self, list_of_developers, prefix=None, expand=False, count=1000, startkey="", format='dict', progress_bar=False
    ):
        apps = {}
        for developer in list_of_developers:
            apps[developer] = self.list_developer_apps(
                developer, prefix=prefix, expand=expand, count=count, startkey=startkey, format=format
            )
        return apps
    ...
    def create_a_consumer_key_and_secret(
        ...
        if not consumer_key:
            consumer_key = "".join(
                random.SystemRandom().choice(string.ascii_letters + string.digits) for _ in range(key_length)
            )
        if not consumer_secret:
            consumer_secret = "".join(
                random.SystemRandom().choice(string.ascii_letters + string.digits) for _ in range(secret_length)
            )
        if key_suffix:
        ...
        if app['credentials']:
            for cred in app['credentials']:
                ...
                resp = self.create_a_consumer_key_and_secret(
                    app['developerId'], consumer_key=consumer_key, consumer_secret=consumer_secret, products=products
                )
