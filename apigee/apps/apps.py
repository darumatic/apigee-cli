...
class Apps:
    ...
    def delete_key_for_a_developer_app(self, developer, consumer_key):
        uri = DELETE_KEY_FOR_A_DEVELOPER_APP_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer, name=self._app_name, consumer_key=consumer_key)
    ...
    def add_api_product_to_key(self, developer, consumer_key, request_body):
        uri = ADD_API_PRODUCT_TO_KEY_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self._org_name, developer=developer, name=self._app_name, consumer_key=consumer_key)
        hdrs = auth.set_header(
            self._auth,
            headers={"Accept": "application/json", "Content-Type": "application/json"},
        )
