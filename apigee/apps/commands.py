...
def _create_developer_app(username, password, mfa_secret, org, profile, name, developer, body, **kwargs):
    return Apps(gen_auth(username, password, mfa_secret), org, None) \
        .create_developer_app(developer, body) \
        .text

@apps.command(help="Creates an app associated with a developer, associates the app with an API product, and auto-generates an API key for the app to use in calls to API proxies inside the API product.")
@common_auth_options
...
def _delete_developer_app(username, password, mfa_secret, org, profile, name, developer, **kwargs):
    return Apps(gen_auth(username, password, mfa_secret), org, name) \
...
@click.option("-n", "--name", help="name", required=True)
@click.option("--developer", help="developer email or id", required=True)
@click.option("--display-name", default=None, help="The DisplayName (set with an attribute) is what appears in the management UI. If you don't provide a DisplayName, the name is used.")
@click.option("--callback-url", default=None, help="The callbackUrl is used by OAuth 2.0 authorization servers to communicate authorization codes back to apps. CallbackUrl must match the value of redirect_uri in some OAuth 2.0 See the documentation on OAuth 2.0 for more details.")
...
@apps.command(help="Get the profile of a specific developer app. All times in the response are UNIX times. Note that the response contains a top-level attribute named accessType that is no longer used by Apigee.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="name", required=True)
...
@apps.command(help="Lists all apps created by a developer in an organization, and optionally provides an expanded view of the apps. All time values in the response are UNIX times. You can specify either the developer's email address or Edge ID.")
@common_auth_options
@common_verbose_options
@common_silent_options
@common_prefix_options
@click.option("--developer", help="developer email or id", required=True)
@click.option('--expand/--no-expand', default=False, help="Set to true to expand the results. This query parameter does not work if you use the count or startKey query parameters.")
@click.option("--count", type=click.INT, default=1000, show_default=True, help="Limits the list to the number you specify. The limit is 100. Use with the startKey parameter to provide more targeted filtering.")
@click.option("--startkey", default="", show_default=True, help="To filter the keys that are returned, enter the name of a developer app that the list will start with.")
...
@apps.command(help="Creates a custom consumer key and secret for a developer app. This is particularly useful if you want to migrate existing consumer keys/secrets to Edge from another system. Consumer keys and secrets can contain letters, numbers, underscores, and hyphens. No other special characters are allowed.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="name", required=True)
...
@click.option("products", "--products", metavar="LIST", cls=OptionEatAll, default=[], help="A list of API products to be associated with the app's credentials")
