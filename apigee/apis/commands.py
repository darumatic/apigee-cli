...
def _delete_api_proxy_revision(username, password, mfa_secret, org, profile, name, revision_number, **kwargs):
    return Apis(gen_auth(username, password, mfa_secret), org) \
        .delete_api_proxy_revision(name, revision_number) \
        .text

@apis.command(help="Deletes a revision of an API proxy and all policies, resources, endpoints, and revisions associated with it. The API proxy revision must be undeployed before you can delete it.")
@common_auth_options
...
def _deploy_api_proxy_revision(
    username, password, mfa_secret, org, profile, name, environment, revision_number, delay=0, override=False, **kwargs
):
    return Apis(gen_auth(username, password, mfa_secret), org) \
...
@click.option("-r", "--revision-number", type=click.INT, help="revision number", required=True)
@click.option("--delay", type=click.INT, default=0, help="Enforces a delay, measured in seconds, before the currently deployed API proxy revision is undeployed and replaced by the new revision that is being deployed. Use this setting to minimize the impact of deployment on in-flight transactions. The default value is 0.")
@click.option('--override/--no-override', default=False, help='Flag that specifies whether to use seamless deployment to ensure zero downtime. Set this flag to "true" to instruct Edge to deploy the new revision fully before undeploying the existing revision. Use in conjunction with the delay parameter to control when the existing revision is undeployed.')
...
def _delete_undeployed_revisions(username, password, mfa_secret, org, profile, name, save_last=0, dry_run=False, **kwargs):
    return Apis(gen_auth(username, password, mfa_secret), org).delete_undeployed_revisions(
...
@common_silent_options
@click.option("-n", "--name", help="name", required=True)
@click.option("--save-last", type=click.INT, help="denotes not to delete the N most recent revisions", default=0)
@click.option('--dry-run/--no-dry-run', default=False, help='show revisions to be deleted but do not delete')
def clean(*args, **kwargs):
    _delete_undeployed_revisions(*args, **kwargs)

def _export_api_proxy(
    username, password, mfa_secret, org, profile, name, revision_number, fs_write=True, output_file=None, **kwargs
):
...
@apis.command(help="Outputs an API proxy revision as a ZIP formatted bundle of code and config files. This enables local configuration and development, including attachment of policies and scripts.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="name", required=True)
@click.option("-r", "--revision-number", type=click.INT, help="revision number", required=True)
@click.option("-O", "--output-file", help="specify output file (defaults to API_NAME.zip)", default=None)
...
@apis.command(help="Gets the names of all API proxies in an organization. The names correspond to the names defined in the configuration files for each API proxy.")
@common_auth_options
@common_verbose_options
@common_silent_options
...
def _undeploy_api_proxy_revision(username, password, mfa_secret, org, profile, name, environment, revision_number, **kwargs):
    return Apis(gen_auth(username, password, mfa_secret), org) \
        .undeploy_api_proxy_revision(
            name, environment, revision_number
        ) \
        .text
...
def _pull(username, password, mfa_secret, org, profile, name, revision_number, environment, work_tree, dependencies=[], force=False, prefix=None, basepath=None, **kwargs):
    return Apis(
        gen_auth(username, password, mfa_secret),
        org,
        revision_number,
        environment,
        work_tree=work_tree,
...
@apis.command(help="""Deploy APIs using an improved version of the Apigee API Proxy Deploy Tool: https://github.com/apigee/api-platform-samples/tree/master/tools
...
@click.option("-e", "--environment", help="environment", required=True)
@click.option("-n", "--name", help="name", required=True)
@click.option('-d', '--directory', help="directory with the apiproxy/ bundle", type=click.Path(exists=True, dir_okay=True, file_okay=False, resolve_path=False), required=True)
@optgroup.group('Deployment options', cls=MutuallyExclusiveOptionGroup,
                help='The deployment options')
@optgroup.option('--import-only/--no-import-only', '-i/-I', default=False, help="import only and not deploy")
@optgroup.option('--seamless-deploy/--no-seamless-deploy', '-s/-S', default=False, help="seamless deploy the bundle")
