...
def _create_keyvaluemap_in_an_environment(username, password, mfa_secret, org, profile, name, environment, body, **kwargs):
    return Keyvaluemaps(gen_auth(username, password, mfa_secret), org, name) \
...
def _delete_keyvaluemap_entry_in_an_environment(username, password, mfa_secret, org, profile, name, environment, entry_name, **kwargs):
    return Keyvaluemaps(gen_auth(username, password, mfa_secret), org, name) \
        .delete_keyvaluemap_entry_in_an_environment(
...
def _get_a_keys_value_in_an_environment_scoped_keyvaluemap(
    username, password, mfa_secret, org, profile, name, environment, entry_name, **kwargs
):
    return Keyvaluemaps(gen_auth(username, password, mfa_secret), org, name) \
        .get_a_keys_value_in_an_environment_scoped_keyvaluemap(
            environment, entry_name
        ) \
        .text
...
def _list_keyvaluemaps_in_an_environment(
    username, password, mfa_secret, org, profile, environment, prefix=None, **kwargs
):
    return Keyvaluemaps(gen_auth(username, password, mfa_secret), org, None).list_keyvaluemaps_in_an_environment(
            environment, prefix=prefix
        )

@keyvaluemaps.command(help="Lists the name of all key/value maps in an environment and optionally returns an expanded view of all key/value maps for the environment.")
@common_auth_options
...
def _create_an_entry_in_an_environment_scoped_kvm(
    username, password, mfa_secret, org, profile, name, environment, entry_name, entry_value, **kwargs
):
    return Keyvaluemaps(gen_auth(username, password, mfa_secret), org, name) \
...
@keyvaluemaps.command(help="Note: This API is supported for Apigee Edge for the Public Cloud only. Creates an entry in an existing KeyValueMap scoped to an environment. A key (name) cannot be larger than 2 KB. KVM names are case sensitive.")
@common_auth_options
@common_silent_options
@common_verbose_options
@click.option("-n", "--name", help="name", required=True)
...
def _update_an_entry_in_an_environment_scoped_kvm(
    username, password, mfa_secret, org, profile, name, environment, entry_name, updated_value, **kwargs
):
    return Keyvaluemaps(gen_auth(username, password, mfa_secret), org, name) \
...
def _list_keys_in_an_environment_scoped_keyvaluemap(
    username, password, mfa_secret, org, profile, name, environment, startkey, count, **kwargs
):
    return Keyvaluemaps(gen_auth(username, password, mfa_secret), org, name) \
        .list_keys_in_an_environment_scoped_keyvaluemap(
            environment, startkey, count
        ) \
        .text

@keyvaluemaps.command(help="Note: This API is supported for Apigee Edge for the Public Cloud only. Lists keys in a KeyValueMap scoped to an environment. KVM names are case sensitive.")
@common_auth_options
# @common_prefix_options
@common_silent_options
@common_verbose_options
...
@click.option("--count", type=click.INT, default=100, show_default=True, help="Limits the list of keys to the number you specify, up to a maximum of 100. Use with the startkey parameter to provide more targeted filtering.")
...
def _push_keyvaluemap(username, password, mfa_secret, org, profile, environment, file, **kwargs):
    return Keyvaluemaps(gen_auth(username, password, mfa_secret), org, None).push_keyvaluemap(
        environment, file
    )

@keyvaluemaps.command(help="Push KeyValueMap to Apigee. This will create KeyValueMap/entries if they do not exist, update existing KeyValueMap/entries, and delete entries on Apigee that are not present in the request body.")
@common_auth_options
@common_silent_options
@common_verbose_options
@click.option("-e", "--environment", help="environment", required=True)
