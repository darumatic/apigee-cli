...
def _clear_all_cache_entries(username, password, mfa_secret, org, profile, name, environment, **kwargs):
    return Caches(gen_auth(username, password, mfa_secret), org, name) \
...
@caches.command(help="Creates a cache in an environment. Caches are created per environment. For data segregation, a cache created in 'test', for example, cannot be accessed by API proxies deployed in 'prod'. The JSON object in the request body can be empty to create a cache with the default settings.")
@common_auth_options
@common_silent_options
@common_verbose_options
@click.option("-n", "--name", help="name", required=True)
...
def _list_caches_in_an_environment(username, password, mfa_secret, org, profile, environment, prefix=None, **kwargs):
    return Caches(gen_auth(username, password, mfa_secret), org, None).list_caches_in_an_environment(
...
def _push_cache(username, password, mfa_secret, org, profile, environment, file, **kwargs):
    return Caches(gen_auth(username, password, mfa_secret), org, None).push_cache(
        environment, file
    )
